...
def contacts(request):
    page = Page.objects.filter(page_type='contacts', active=True).first()
    return render(request, 'website/contacts.html', {'content': page})

def privacy_policy(request):
    page = Page.objects.filter(page_type='privacy-policy', active=True).first()
    return render(request, 'website/privacy-policy.html', {'content': page})

def coupons(request):
    page = Page.objects.filter(page_type='coupons', active=True).first()
    return render(request, 'website/coupons.html', {'content': page})
